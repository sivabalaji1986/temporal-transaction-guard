...
**Architecture:** An upstream fraud engine identifies candidate transactions that may require further action and hands each one off with a `fraudScore`/`triggerReason`/`customerId` — this project does not recompute that score. A FastAPI app starts a Temporal workflow per transaction. The workflow does its own deterministic threshold check, then (if at/above threshold) places the hold, calls a pydantic-ai/Ollama activity to generate customer/ops explanations, notifies the customer, and durably waits up to 24h for a `customer_responded` signal before resolving to release/block/escalate. A separate worker process executes the workflow and all activities; killing and restarting it mid-hold must preserve the workflow's progress and rely on idempotent activities to avoid duplicate real-world side effects.

> Narrative note (2026-08-10): the framing above was clarified post-launch to make the upstream boundary explicit — the fraud engine identifies *candidates*, it does not itself decide to hold. The task-by-task content below (including embedded code snippets, e.g. Task 5's `generate_explanation` system prompt) reflects the plan as executed on 2026-08-04 and is intentionally left as a historical record rather than rewritten — see `AGENTS.md` and the current source for up-to-date wording.
...
- **No tests beyond five workflow tests**: the original four specified in Task 6 (below-threshold, it_was_me, not_me, timeout→escalate), plus one covering the `generate_explanation` failure/fallback path (added post-launch to close a real reliability gap — see `app/workflows/fraud_hold_workflow.py`'s `except ActivityError` handling). Do not add tests for models, config, individual activities, or the FastAPI layer — verify those manually (a `python -c` snippet, `docker compose config`, or `curl`), per the spec's explicit scope limit. The five-test count is deliberate and should not be flagged as scope creep; the original four-test rule existed to block low-value test sprawl, not to block coverage of a genuine workflow-level correctness gap. **Superseded (2026-08-11):** once `generate_explanation` became a tool-using Agent, this "no activity-internals tests" constraint was deliberately relaxed — see `tests/test_generate_explanation_agent.py` (seven tests) and `AGENTS.md`. The suite is now 12 tests total; this bullet's *reasoning* (don't add tests for a real correctness gap without saying so explicitly) still applies, its specific "five, no activity-internals" boundary does not.
...
- **Spec coverage:** every spec section maps to a task — scaffolding/pins (Task 1), models (Task 2), config (Task 3), mocked activities (Task 4), `generate_explanation` (Task 5), workflow + tests (Task 6, originally 4, plus a 5th added post-launch for the `generate_explanation` failure/fallback path — see Global Constraints), worker (Task 7), FastAPI + idempotency (Task 8), `send_signal.py` (Task 9), docker-compose crash-resume demo (Task 10).
