...
- **`model_config = SettingsConfigDict(env_file=".env", ...)`** — tells it to also read from a `.env` file if one exists (that's the file `README.md`'s Setup instructions have you copy from `.env.example`). Worth knowing: this reads `.env` from wherever the process's current working directory happens to be, not from a path relative to `config.py` itself — so it applies equally to `docker-compose.yml`'s containers and to a native `pytest tests/` run from the repo root.
...
- **`demo_failover_delay_seconds` and `demo_model_retry_interval_seconds`** — two demo/observability-only knobs, both `0`/`1` by default (matching normal behavior exactly), and both read by `generate_explanation.py`: the first delays `lookup_customer_channel_preference` (Section 2.6) for the manual two-Worker failover demo; the second controls the backoff between model-request Activity retry attempts, which is also part of the worst-case-timing calculation in `AGENTS.md` and README's "Timeout and retry budget." Because `Settings` reads whatever `.env` is sitting in the current working directory (see above), leaving either one set to a non-default value in your local `.env` after running a demo will keep affecting *every* later run from that directory — including a plain `pytest tests/` — until you set it back.
...
- **`initial_interval=timedelta(seconds=settings.demo_model_retry_interval_seconds)` — why does the model-request backoff come from `settings` instead of a plain `1`?** `settings.demo_model_retry_interval_seconds` defaults to `1`, matching the tool-call config's backoff and the 810s calculation above exactly — but making it a setting (via `DEMO_MODEL_RETRY_INTERVAL_SECONDS`) lets a demo operator widen the gap between a failed model-request attempt and its retry, the same way `demo_failover_delay_seconds` widens the window on a tool call (Section 2.6, below). It's a demo/observability knob, not a production tuning parameter: raising it raises the 810s worst case and requires recalculating, and — because `Settings` reads `.env` from the process's working directory (Section 2.2) — leaving it raised in your local `.env` after a demo keeps affecting later runs, including `pytest`, from that same directory. It doesn't *cause* a retry — the automated test in `tests/test_generate_explanation_agent_durability.py` (Section 2.13) remains the deterministic, reproducible proof that a completed tool Activity survives a later model-request Activity failing and retrying. This setting only widens the window *if* a real model-request Activity happens to fail against live Ollama, so README's live "Demo B" walkthrough has a realistic chance of catching it in the Web UI.
...
- **`Client.connect(...)`** — opens a connection to the Temporal *server* (a separate piece of software this project runs via Docker — see `README.md`). Everything this project does with Temporal goes through a `Client` like this one.
...
**Called by:** nobody (in Python terms) — it's started directly as a process, via `docker-compose.yml`'s `worker` service (`command: python -m app.worker`), which can be scaled to run several of these processes at once — see README's "Demo C".
...
- **Scripting a mid-loop failure, then checking two different kinds of evidence** — the scripted script has the model call `lookup` on its first turn, then deliberately fail (raise an exception) on its *second* turn once, succeeding only on the retry. This test checks two independent kinds of evidence that the already-completed `lookup` call is *not* re-invoked when that later model-request Activity fails and retries: `_TOOL_CALL_LOG == ["lookup(CUST-101)"]` (the tool's own Python-level call count — a plain list a real invocation appends to, checked exactly, not just "at least once"), *and* `handle.fetch_history_events(...)` — a direct query against the Temporal server's own recorded Event History (Section 4.1) — counted for `ActivityTaskScheduled` events per Activity type name. Checking both matters: the first proves the tool genuinely wasn't re-invoked at the Python level; the second proves that's not a coincidence of this particular mock, but is exactly what Temporal itself records as having happened.
- **`ActivityTaskScheduled` counts logical Activities, not attempts — why does `model_schedules == 2` prove the retry, rather than `3`?** `ActivityTaskScheduled` is recorded once per *logical* Activity invocation — here, once per genuine Agent turn: turn 1's model request (succeeds on its first attempt) and turn 2's model request (needs a second attempt after its first one fails). That's 2 schedules total, matching `model_schedules == 2`. The retry itself doesn't add a third schedule: a failed attempt that gets retried shows up as an additional `ActivityTaskStarted` under the *same* schedule, not a new `ActivityTaskScheduled`. Meanwhile `tool_schedules == 1` confirms `lookup` was scheduled exactly once, proving it wasn't re-scheduled (and therefore wasn't re-invoked) when that later model-request Activity's first attempt failed and had to retry. In short: `ActivityTaskScheduled` count = number of logical Activity invocations; retries of any one of those show up as extra `ActivityTaskStarted` attempts under its existing schedule, never as their own schedule.
...
**What it does:** a Signal is how something *outside* a running workflow — here, an HTTP request or a CLI script — delivers a message *into* it asynchronously, without needing to know or care whether the workflow happens to be actively running or durably paused at that exact moment, or even whether a worker process is running at all. Temporal's server accepts and durably records the Signal either way — no worker needs to be polling the task queue at the moment it arrives, only by the time the workflow needs to act on it. README's "Demo A" is the manual proof of exactly this: stop the worker, send the Signal while it's down (the server records it; the workflow stays `Running`, not failed, even if you see a `SCHEDULE_TO_START` Workflow Task timeout in the meantime), then restart the worker and watch it process the already-recorded Signal and history normally.
